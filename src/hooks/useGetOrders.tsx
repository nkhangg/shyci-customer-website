...
import { IDOrder, IDProduct, IFilterOrder, IFilterProduct } from '../../interface';
...
export interface IuseGetOrdersProps {
    queries?: IFilterOrder;
}

export default function useGetOrders({ queries }: IuseGetOrdersProps) {
    ...
    const { data, isLoading, hasNextPage, isFetching, isFetchingNextPage, fetchNextPage, refetch, ...prev } = useInfiniteQuery({
        queryKey: ['useGetOrders', { ...queries, search: searchD }],
        queryFn: ({ pageParam = 1 }) => {
            return getOrders({ page: pageParam, ...queries, search: searchD });
        },
        initialPageParam: 1,
    ...
    const result: IDOrder[] = data?.pages.reduce((item: any, cur) => {
